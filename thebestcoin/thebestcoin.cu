...
extern void thebestcoin_cpu_hash_32(int thr_id, uint32_t threads, uint64_t *dOutputHash);
extern void thebestcoin_cpu_init(int thr_id, uint32_t threads);
...
extern "C" int scanhash_thebestcoin(
	int thr_id,
	uint32_t *pdata,
	...
	unsigned int vram = 1024 * 1024 * 1024;							// 1GB VRAM
	unsigned int msize = BLOCK_LEN_INT64 * 8 * N_COLS * N_ROWS;		// Matrix size

	uint32_t intensity = (vram / msize) ;
	uint32_t throughput = device_intensity(device_map[thr_id], __func__, intensity);

	max_nonce = throughput;
	...
	if (!init[thr_id])
	{
		...
		thebestcoin_cpu_init(thr_id, throughput);
	...
	do {
		...
		thebestcoin_cpu_hash_32(thr_id, throughput, d_hash[thr_id]);
