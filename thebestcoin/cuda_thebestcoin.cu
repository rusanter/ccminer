...
__device__ __forceinline__ void TB_Gfunc_v35(uint2 & a, uint2 &b, uint2 &c, uint2 &d)
{
...
__device__ __forceinline__ void TB_round_lyra_v35(vectype* s)
{

	TB_Gfunc_v35(s[0].x, s[1].x, s[2].x, s[3].x);
	TB_Gfunc_v35(s[0].y, s[1].y, s[2].y, s[3].y);
	TB_Gfunc_v35(s[0].z, s[1].z, s[2].z, s[3].z);
	TB_Gfunc_v35(s[0].w, s[1].w, s[2].w, s[3].w);

	TB_Gfunc_v35(s[0].x, s[1].y, s[2].z, s[3].w);
	TB_Gfunc_v35(s[0].y, s[1].z, s[2].w, s[3].x);
	TB_Gfunc_v35(s[0].z, s[1].w, s[2].x, s[3].y);
	TB_Gfunc_v35(s[0].w, s[1].x, s[2].y, s[3].z);
...
__device__ __forceinline__ void TB_reduceDuplex50(vectype state[4], uint32_t thread)
{
...
__device__  void TB_reduceDuplexRowSetupV2(const int rowIn, const int rowInOut, const int rowOut, vectype state[4], uint32_t thread)
{
...
__device__ void TB_reduceDuplexRowtV2(const int rowIn, const int rowInOut, const int rowOut, vectype* state, uint32_t thread)
{
	int i,j;
		vectype state2[3];
		const uint32_t ps1 = (memshift * Ncol * rowIn + Nrow * Ncol * memshift * thread);
...
#pragma unroll
		...
		TB_reduceDuplex50(state, thread);

		TB_reduceDuplexRowSetupV2(1, 0, 2, state,  thread);
		TB_reduceDuplexRowSetupV2(2, 1, 3, state,  thread);
        ...
        for (int i = 0; i < 4; i++)
        {
		 ...
		 TB_reduceDuplexRowtV2(prev, rowa, i, state, thread);
         prev=i;
        }
